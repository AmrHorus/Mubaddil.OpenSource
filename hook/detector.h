 ...
 * Supports "Did You Mean?" popup functionality.
 */
...
struct CorrectionCandidate {
    ...
    double convertedConfidence; ///< Confidence of converted word
    bool shouldSuggest;         ///< Whether to show suggestion
    
    CorrectionCandidate()
        : detectedLang(LanguageType::Unknown)
        , confidence(0.0), convertedConfidence(0.0), shouldSuggest(false) {}
};
 ...
 * Implements "Did You Mean?" popup logic.
 */
class LanguageDetector {
public:
    /**
    ...
    /**
     * @brief Evaluate a word for potential keyboard layout mistake
     * @param word The completed word to evaluate
     * @param converted The word converted to opposite layout
     * @return CorrectionCandidate with evaluation results
     */
    CorrectionCandidate EvaluateWord(const std::wstring& word, 
                                     const std::wstring& converted);
    
    /**
     * @brief Set the confidence threshold for showing suggestions
     * @param threshold Minimum confidence difference required (0.0-1.0)
     */
    ...
    /**
     * @brief Check if a word is a common word in a specific language
     * @param word Word to check
     * @param lang Language to check against
     * @return true if word is common in that language
     */
    bool IsCommonWord(const std::wstring& word, LanguageType lang);
...
private:
    ...
    std::unordered_map<std::wstring, bool> m_commonEnglishWords; ///< Common English words cache
    std::unordered_map<std::wstring, bool> m_commonArabicWords;  ///< Common Arabic words cache
    double m_threshold;                                    ///< Confidence threshold for suggestions
};
